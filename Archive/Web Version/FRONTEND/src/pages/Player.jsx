...
export default function Player(query) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${BACKEND_URL}/add/${encodeURIComponent(magnetURI)}`,
      );
      console.log(response);
      // Step 2: Set the video source for streaming
      setVideoSrc(
        `${BACKEND_URL}/stream/${encodeURIComponent(magnetURI)}`,
      );
      // Step 3: Set the subtitle source
      setSubtitleSrc(
        `${BACKEND_URL}/subtitles/${encodeURIComponent(magnetURI)}`,
      );
    } catch (error) {
  ...
  const handleStreamBrowser = (eipsode) => {
    setVideoSrc(
      `${BACKEND_URL}/streamfile/${encodeURIComponent(magnetURI)}/${encodeURIComponent(eipsode)}`,
    );
  };
  ...
  const stopEpisodeDownload2 = async (episode) => {
    try {
      ...
      console.log(
        `${BACKEND_URL}/deselect/${encodeURIComponent(magnetURI)}/${encodeURIComponent(episode)}`,
      );
  ...
  const handleRemoveTorrent = async () => {
    try {
      // Send a DELETE request to remove the torrent
      await axios.delete(
        `${BACKEND_URL}/remove/${encodeURIComponent(magnetURI)}`,
      );
